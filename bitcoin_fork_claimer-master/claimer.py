...
def get_tx_details_from_blockchaininfo(txid, addr, hardforkheight):
    asd=1#print "Querying blockchain.info API about data for transaction %s" % txid
    ...
    if hardforkheight < txinfo["block_height"]:
        ...
        asd=1#print "(fork at height %d, this tx at %d)" % (hardforkheight, txinfo["block_height"])
        asd=1#print "You will most likely be unable to claim these coins."
        asd=1#print "Please look for an earlier transaction before the fork point.\n\n"
    ...
    for outinfo in txinfo["out"]:
        if "addr" in outinfo and outinfo["addr"] == addr:
            ...
            asd=1#print "Candidate transaction, index %d with %d Satoshis (%.8f BTC)" % (txindex, satoshis, satoshis / 100000000.0)
            if found is None:
...
def get_btx_details_from_chainz_cryptoid(addr):
    asd=1#print "Querying chainz.cryptoid.info about last unspent transaction for address"
...
def get_coin_details_from_electrum(coin, targettxid, sourceh160, keytype):
    ...
    if len(unspents) == 1:
        ...
        if target is None:
            asd=1#print "Multiple potential outputs possible - please use one of these TXIDs to claim"
            for tx in unspents:
                coinamount = int(tx["value"]) * coin.coinratio / 100000000.0
                btcamount = int(tx["value"]) / 100000000.0
                asd=1#print "    TXID %s : %20.8f %s (equivalent to %.8f BTC)" % (tx["tx_hash"], coinamount, coin.ticker, btcamount)
...
class Client(object):
    ...
    def send(self, cmd, msg):
        ...
        asd=1#print "---> %s (%d bytes)" % (repr(cmd), len(msg))
    ...
    def send_tx(self, txhash, tx, checkfee):
        ...
        while True:
            try:
                ...
                asd=1#print "Connecting to", address, "...",
                self.sc = socket.create_connection(address, 10)
                asd=1#print "SUCCESS, connected to", self.sc.getpeername()
                ...
                while True:
                    ...
                    asd=1#print "<--- '%s' (%d bytes)" % (cmd, len(payload))
                    if cmd == "version":
                        ...
                        asd=1#print "     Version information:"
                        asd=1#print "\tprotocol version", protoversion
                        asd=1#print "\tservices", services
                        asd=1#print "\ttimestamp", time.asctime(time.gmtime(timestamp))
                        asd=1#print "\tuser agent", repr(user_agent)
                        asd=1#print "\tblock height", repr(start_height)
                        client.send("verack", "")
                        ...
                        if not self._transaction_sent:
                            ...
                            client.send("tx", tx)
                            print txhash[::-1].encode("hex")
                            os._exit(1)
                            asd=1#print "\tRe-sent transaction: %s" % txhash_hexfmt

                        client.send("mempool", "")
                        
                    elif cmd == "getdata":
                        if payload == "\x01\x01\x00\x00\x00" + txhash:
                            asd=1#print "\tPeer requesting transaction details for %s" % txhash_hexfmt
                            client.send("tx", tx)
                            print txhash[::-1].encode("hex")
                            os._exit(1)
                            asd=1#print "\tSENT TRANSACTION: %s" % txhash_hexfmt
                        ...
                        # If a getdata comes in without our txhash, it generally means the tx was rejected.
                            ...
                            asd=1#print "\tReceived getdata without our txhash. The transaction may have been rejected."
                            asd=1#print "\tThis script will retransmit the transaction and monitor the mempool for a few minutes before giving up."
                         
                    elif cmd == "feefilter":
                        minfee = struct.unpack("<Q", payload)[0]
                        asd=1#print "\tserver requires minimum fee of %d satoshis" % minfee
                        if checkfee >= minfee:
                            asd=1#print "\tour fee is >= minimum fee, so should be OK"
                        else:
                            asd=1#print "\tOUR FEE IS TOO SMALL, transaction might not be accepted"
                        ...
                        for i in xrange(ninv):
                            ...
                            if i < 10:
                                asd=1#print "\t%s: %s" % (invtypestr, invhash[::-1].encode("hex"))
                            elif i == 10:
                                asd=1#print "\t..."
                                asd=1#print "\tNot asd=1#printing additional %d transactions" % (ninv - i)
                        ...
                        if transaction_found and not self._transaction_accepted:        
                            ...
                            # User specified --noblock, we are done here
                                ...
                                asd=1#print "\tConsider leaving this script running until it detects the transaction in a block."
                            self._transaction_accepted = True
                        elif transaction_found:
                            asd=1#print "\tTransaction still in mempool. Continue waiting for block inclusion."
                            ...
                            if self._mempool_check_count <= self._MAX_MEMPOOL_CHECKS:
                                asd=1#print "\tWill retransmit and check again %d more times." % (self._MAX_MEMPOOL_CHECKS - self._mempool_check_count)
                            else:
                        ...
                        if blocks_to_get:
                            ...
                            asd=1#print "\trequesting %d blocks" % len(blocks_to_get)
                        
                    elif cmd == "block":
                        if tx in payload or plaintx in payload:
                            asd=1#print "\tBLOCK WITH OUR TRANSACTION OBSERVED! YES!"
                            asd=1#print "\tYour coins have been successfully sent. Exiting..."
                            return
                        else:
                            asd=1#print "\tTransaction not included in observed block."
                        ...
                        for _ in xrange(naddr):
                            ...
                            asd=1#print "\tGot peer address: %s" % address
                    elif cmd not in ('sendcmpct', 'verack'):
                        asd=1#print repr(cmd), repr(payload)
                
            except (socket.error, socket.herror, socket.gaierror, socket.timeout) as e:
                if self._connection_retries >= self._MAX_CONNECTION_RETRIES:
                    raise
                asd=1#print "\tConnection failed with: %s" % repr(e)
                asd=1#print "\tWill retry %d more times." % (self._MAX_CONNECTION_RETRIES - self._connection_retries)
...
if args.height and coin.hardforkheight < args.height:
    ...
    asd=1#print "(fork at height %d)" % coin.hardforkheight
    exit()
...
if remaining <= 0:
    asd=1#print "The specified amount of satoshis specified is smaller than or equal to the fee."
    asd=1#print "Note that the '--satoshis' parameter needs to be the TOTAL amount available in the source transaction."
    asd=1#print "If you want a custom fee, use '--fee'."
...
for output in args.destaddr.split(","):
    if ":" not in output:
        ...
        amount = int(amount)
        if amount > remaining:
            raise Exception("Specified satoshis amount ("+str(amount)+") exceeds remaining balance ("+str(remaining)+")")
        
    if destaddr.startswith("bc1"):
        rawaddr = bech32decode(destaddr)
        assert len(rawaddr) == 20
        asd=1#print "YOU ARE TRYING TO SEND TO A bech32 ADDRESS! THIS IS NOT NORMAL! Are you sure you know what you're doing?"
        ...
        if rawaddr[0] == "\x00" or rawaddr.startswith(coin.PUBKEY_ADDRESS):
            ...
            asd=1#print "YOU ARE TRYING TO SEND TO A P2SH ADDRESS! THIS IS NOT NORMAL! Are you sure you know what you're doing?"
...
asd=1#print "Raw transaction"
asd=1#print tx.encode("hex")
asd=1#print

coinamount = satoshis * coin.coinratio / 100000000.0
btcamount = satoshis / 100000000.0
asd=1#print "YOU ARE ABOUT TO SEND %.8f %s (equivalent to %.8f BTC) FROM %s" % (coinamount, coin.ticker, btcamount, args.srcaddr)

for outscript, amount, destaddr, rawaddr in outputs:
    coinamount = amount * coin.coinratio / 100000000.0
    btcamount = amount / 100000000.0
    asd=1#print "    %.8f %s (equivalent to %.8f BTC) TO %s" % (coinamount, coin.ticker, btcamount, destaddr)
    
coinamount = args.fee * coin.coinratio / 100000000.0
btcamount = args.fee / 100000000.0
asd=1#print "!!! %.8f %s (equivalent to %.8f BTC) WILL BE SENT AS FEES! CONTINUE AT YOUR OWN RISK !!!" % (coinamount, coin.ticker, btcamount)
...
if coin.ticker == "BTP":
    ...
    try:
        ...
        print txhash[::-1].encode("hex")
        os._exit(1)
        asd=1#print "received data", repr(data)
        data = json.loads(data)
        if data["txid"] == txhash[::-1].encode("hex"):
            asd=1#print "Pushed transaction successfully!"
            asd=1#print "This does NOT mean the transaction will happen, just that the signature is valid."
            asd=1#print "All you can do now is wait."
        else:
            asd=1#print "Server says transaction push failed!", repr(data)
            asd=1#print "Transaction might still have been accepted, wait for a few minutes to see if it arrives."
    except urllib2.HTTPError, e:
        asd=1#print "API gave error", e
        asd=1#print repr(e.read())
    ...
    try:
        ...
        asd=1#print "Success - server accepted transaction and responded with TXID %s" % j["result"]
    except:
        asd=1#print "ERROR when submitting transaction!"
        asd=1#print "Raw server response: %r" % res
